...
class Sample:
    ...
    def get_srcdst_port_stats(self, src, dst):
        """
        ...
        Returns
        -------
        Dictionary with the performance metrics of the interface port of node src
        that is connected with dst node.
        None if no link exist between src and dst
        ...
        if dst in self.port_stats[src]:
            res = self.port_stats[src][dst]
    ...
    def _process_link_usage_line(self, lline,s):
        ...
        for i in range(netSize):
            ...
            for j in range(netSize):
                ...
                link_stat["qosQueuesState"] = qos_queue_stat_lst;
